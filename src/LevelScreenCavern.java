...
public class LevelScreenCavern extends BaseScreen{
    Player band;
    ...
    public void initialize() {    
        ...
        TilemapActor tma = new TilemapActor("assets/mapaCavern.tmx", mainStage);
        ...
        music = Gdx.audio.newMusic(Gdx.files.internal("assets/Robobozo.mp3"));
        ...
        for (MapObject obj : tma.getTileList("Enemy6") ){
        	
            MapProperties props = obj.getProperties();
            new Enemy6( (float)props.get("x"), (float)props.get("y"), mainStage );
        }
        ...
        for (MapObject obj : tma.getTileList("Enemy7") ){
        	
            MapProperties props = obj.getProperties();
            new Enemy7( (float)props.get("x"), (float)props.get("y"), mainStage );
        }
   ...
   /*     for (BaseActor timer : BaseActor.getList(mainStage, "Timer")){
        ...
        for (BaseActor actor : BaseActor.getList(mainStage, "Solid")){
            ...
            /**
             * Colisiones del Enemigo6 con el Suelo
             */
            for (BaseActor enemy6 : BaseActor.getList(mainStage, "Enemy6")){
            	
                if (enemy6.overlaps(solid)){
                	enemy6.preventOverlap(solid);
                	enemy6.preventOverlap(solid);
                	enemy6.setMotionAngle( enemy6.getMotionAngle() + 180 );
                }
            }
            /**
             * Colisiones del Enemigo7 con el Suelo
             */
            for (BaseActor enemy7 : BaseActor.getList(mainStage, "Enemy7")){
            	
            	if (enemy7.overlaps(solid)){
            		enemy7.preventOverlap(solid);
		        	if(enemy7.velocityVec.x > 0) {
		        		enemy7.velocityVec.x = -(MathUtils.random(60,100));
		        		enemy7.setScaleX(1);
        ...
        /**
         * Colisiones del Enemigo6 con Colisiones para que no se salgan
         */
        for (BaseActor enemy1Solid : BaseActor.getList(mainStage, "Enemy1Solid")){    

            for (BaseActor enemy6 : BaseActor.getList(mainStage, "Enemy6")){
            	
                if (enemy6.overlaps(enemy1Solid)){
                	
                	enemy6.preventOverlap(enemy1Solid);
                	enemy6.setMotionAngle( enemy6.getMotionAngle() + 180 );
                }
            }
        }
        ...
        /**
         * Colision del Enemigo2 y el Enemigo7
         */
        for (BaseActor enemy2 : BaseActor.getList(mainStage, "Enemy2")){    

            for (BaseActor enemy7 : BaseActor.getList(mainStage, "Enemy7")){
            	
                if (enemy2.overlaps(enemy7)){
        ...
        /**
         * Colision del Enemigo7 y el Enemigo2
         */
        for (BaseActor enemy7 : BaseActor.getList(mainStage, "Enemy7")){    

            for (BaseActor enemy2 : BaseActor.getList(mainStage, "Enemy2")){
            	
            	if(enemy7.overlaps(enemy2)){
                	
                	if(enemy7.velocityVec.x > 0) {
                		enemy7.velocityVec.x = -(MathUtils.random(50,65));
                		enemy7.setScaleX(1);
        ...
        /**
         * Colision del Enemigo6 y el Enemigo2
         */
        for (BaseActor enemy6 : BaseActor.getList(mainStage, "Enemy6")){    

            for (BaseActor enemy2 : BaseActor.getList(mainStage, "Enemy2")){
            	
                if (enemy6.overlaps(enemy2)){
                	
                	if(enemy6.velocityVec.x > 0) {
                		enemy6.setMotionAngle( enemy6.getMotionAngle() - 180 );             
		            }else {
		            	enemy6.setMotionAngle( enemy6.getMotionAngle() + 180 );
		            }    	
        ...
        /**
         * Colision del Enemigo6 y el Enemigo3
         */
        for (BaseActor enemy6 : BaseActor.getList(mainStage, "Enemy6")){    

            for (BaseActor enemy3 : BaseActor.getList(mainStage, "Enemy3")){
            	
            	if(enemy6.overlaps(enemy3)){
        ...
        /**
         * Colision del Enemigo3 y el Enemigo6
         */
        for (BaseActor enemy3 : BaseActor.getList(mainStage, "Enemy3")){    

            for (BaseActor enemy6 : BaseActor.getList(mainStage, "Enemy6")){
            	
                if (enemy3.overlaps(enemy6)){
        ...
        /**
         * Colision del Enemigo7 y el Enemigo6
         */
        for (BaseActor enemy7 : BaseActor.getList(mainStage, "Enemy7")){    

            for (BaseActor enemy6 : BaseActor.getList(mainStage, "Enemy6")){
            	
                if (enemy7.overlaps(enemy6)){
        ...
        /**
         * Colision del Enemigo2 y el Enemigo6
         */
        for (BaseActor enemy2 : BaseActor.getList(mainStage, "Enemy2")){    

            for (BaseActor enemy6 : BaseActor.getList(mainStage, "Enemy6")){
            	
            	if(enemy2.overlaps(enemy6)){
        ...
        /**
         * Colisiones del Enemigo6 con el Jugador
         */
        for (BaseActor enemy6 : BaseActor.getList(mainStage, "Enemy6")){
            if ( band.overlaps(enemy6) ){
            		
            	if(recibirDaño == true) {
            		
            		band.preventOverlap(enemy6);  
                	enemy6.setMotionAngle( enemy6.getMotionAngle() + 180 );
                    Vector2 bandPosition  = new Vector2(  band.getX(),  band.getY() );
                    Vector2 enemy1Position = new Vector2( enemy6.getX(), enemy6.getY() );
        ...
        /**
         * Colisiones del Enemigo7 con el Jugador
         */
        for (BaseActor enemy7 : BaseActor.getList(mainStage, "Enemy7")){
            if ( band.overlaps(enemy7) ){
            	    
            	if(recibirDaño == true) {
            		
            		band.preventOverlap(enemy7); 
         ...
         * Matar enemigo3 al saltar
         */
        for (BaseActor enemigo7 : BaseActor.getList(mainStage, "Enemy7")){
        	
            if (band.belowOverlaps(enemigo7)){
            	
            	enemigo7.remove();
                
                if ( MathUtils.random(1,2) == 1 ) {
                	Fireballitem fireballitem = new Fireballitem(0,0, mainStage);
                    fireballitem.centerAtActor(enemigo7);
                    fireballitem.centerAtActor(enemigo7); 
                }else {
                	Fireballitem fireballitem = new Fireballitem(0,0, mainStage);
                    fireballitem.centerAtActor(enemigo7); 
        ...
        for (BaseActor fireball : BaseActor.getList(mainStage, "Fireball")){
            ...
            for (BaseActor enemy6 : BaseActor.getList(mainStage, "Enemy6")){
            	...
            	if (fireball.overlaps(enemy6)){
                	
            		enemy6.remove();
                    fireball.remove();
                    Coin coin = new Coin(0,0, mainStage);
                    coin.centerAtActor(enemy6);                    
                }
            }	
        	...
        	for (BaseActor enemy7 : BaseActor.getList(mainStage, "Enemy7")){
            	...
            	if (fireball.overlaps(enemy7)){
                	
            		enemy7.remove();
                    fireball.remove();
                    if ( MathUtils.random(1,6) == 1 ) {
                    	TripleCoin coinx3 = new TripleCoin(0, 0, mainStage);
                    	coinx3.centerAtActor(enemy7);
                    }else {
                    	Coin coin = new Coin(0,0, mainStage);
                        coin.centerAtActor(enemy7); 
                    }
